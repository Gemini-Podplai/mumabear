...
return (
  <div className={`h-screen flex flex-col ${theme === 'comfort'
      ...
      <div className={`p-4 border-b ${theme === 'custom' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-purple-50'}`}>
        ...
        >
          {researchTools.map((tool, index) => (
            <div
              key={tool.id}
              className={`p-4 rounded-xl cursor-pointer transition-all duration-300 transform hover:scale-105 ${selectedSource === tool.id.split('_')[0]
            ...
            >
              <div className="text-center">
                <div className={`w-12 h-12 mx-auto mb-3 rounded-lg flex items-center justify-center ${selectedSource === tool.id.split('_')[0]
                    ? `bg-${tool.color}-200`
                    : theme === 'custom'
                      ? 'bg-gray-600'
                      : 'bg-gray-100'
                  }`}>
                  <tool.icon className={`w-6 h-6 ${selectedSource === tool.id.split('_')[0]
                      ? `text-${tool.color}-600`
                      : theme === 'custom' ? 'text-gray-300' : 'text-gray-600'
                    }`} />
                </div>

                <h3 className={`font-semibold text-sm mb-1 ${theme === 'custom' ? 'text-white' : 'text-gray-900'
                  }`}>
                  {tool.name}
                </h3>
                ...
                <div className="mt-2 flex justify-center">
                  <div className="flex space-x-1">
                    {tool.capabilities.slice(0, 3).map((_, idx) => (
                      <div
                        key={idx}
                        className={`w-1.5 h-1.5 rounded-full ${selectedSource === tool.id.split('_')[0]
                            ? `bg-${tool.color}-400`
                            : 'bg-gray-300'
                          }`}
                      />
    ...
    <div className={`p-4 border-b ${theme === 'custom' ? 'border-gray-700' : 'border-gray-200'}`}>
      <form onSubmit={handleSearch} className="max-w-4xl mx-auto">
        <div className="flex items-center space-x-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className={`w-full pl-12 pr-4 py-3 rounded-xl border focus:ring-2 focus:ring-purple-500 focus:border-transparent ${theme === 'custom'
          ...
          <button
            ...
            className={`px-6 py-3 rounded-xl transition-all duration-200 flex items-center gap-2 ${currentQuery.trim()
        ...
        <div className="mt-2 flex items-center justify-center">
          <span className={`text-sm px-3 py-1 rounded-full ${theme === 'custom'
    ...
    <div className="flex-1 flex">
      ...
      {showSideTickers && (
        <>
          ...
          <div className={`w-64 border-r ${theme === 'custom' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'
            } p-4 overflow-y-auto`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className={`p-1 rounded ${activePreview === 'search1' ? 'bg-purple-100 text-purple-600' : ''
                  }`}
              >
                <Eye size={16} />
              </button>
            ...
            <div className="space-y-3">
              {liveSearches.slice(0, 5).map((query, index) => (
                <div
                  key={query.id}
                  className={`p-3 rounded-lg cursor-pointer transition-all ${theme === 'custom'
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <span className={`text-xs px-2 py-1 rounded-full ${query.status === 'completed' ? 'bg-green-100 text-green-700' :
                  ...
                  <p className={`text-sm font-medium mb-1 line-clamp-2 ${theme === 'custom' ? 'text-white' : 'text-gray-900'
                    }`}>
                    {query.query}
                  </p>
          ...
          <div className={`w-64 border-l ${theme === 'custom' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50'
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className={`p-1 rounded ${activePreview === 'search2' ? 'bg-purple-100 text-purple-600' : ''
                  }`}
              >
                <Eye size={16} />
              </button>
            ...
            <div className="space-y-3">
              {mockResults.map((result, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <span className={`text-xs px-2 py-1 rounded-full ${result.type === 'academic' ? 'bg-blue-100 text-blue-700' :
                        result.type === 'documentation' ? 'bg-orange-100 text-orange-700' :
                          'bg-green-100 text-green-700'
                      }`}>
                      {result.type}
                    </span>
                    <span className={`text-xs font-mono ${result.relevance > 90 ? 'text-green-600' :
                  ...
                  <h4 className={`text-sm font-medium mb-1 line-clamp-2 ${theme === 'custom' ? 'text-white' : 'text-gray-900'
                    }`}>
                    {result.title}
                  </h4>

                  <p className={`text-xs line-clamp-2 ${theme === 'custom' ? 'text-gray-400' : 'text-gray-600'
                    }`}>
                    {result.snippet}
                  </p>
      ...
      <div className="flex-1 flex flex-col">
        {/* Master view header */}
        <div className={`flex items-center justify-between p-4 border-b ${theme === 'custom' ? 'border-gray-700' : 'border-gray-200'
          }`}>
          <div className="flex items-center space-x-3">
            ...
            <span className={`text-sm px-3 py-1 rounded-full ${theme === 'custom' ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-600'
              }`}>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${theme === 'custom' ? 'hover:bg-gray-700' : 'hover:bg-gray-100'
                }`}
            >
              <Maximize2 size={16} />
            </button>
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      <div className={`w-3 h-3 rounded-full ${query.status === 'completed' ? 'bg-green-400' :
                    ...
                    <div className="flex items-center space-x-4 text-sm">
                      <span className={`px-2 py-1 rounded-full ${theme === 'custom'
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {query.results.map((result) => (
                    <div
                      key={result.id}
                      className={`p-4 rounded-lg cursor-pointer transition-all hover:scale-105 ${theme === 'custom'
                          ? 'bg-gray-700 hover:bg-gray-600'
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className={`text-xs font-mono ${result.relevance > 90 ? 'text-green-600' :
                            result.relevance > 75 ? 'text-yellow-600' :
                              'text-gray-500'
                          }`}>
                          {Math.round(result.relevance)}%
                      ...
                      <h4 className={`font-semibold mb-2 line-clamp-2 ${theme === 'custom' ? 'text-white' : 'text-gray-900'
                        }`}>
                        {result.title}
                      </h4>

                      <p className={`text-sm mb-3 line-clamp-3 ${theme === 'custom' ? 'text-gray-300' : 'text-gray-600'
                        }`}>
                        {result.snippet}
                      </p>

                      <div className={`text-xs flex items-center justify-between ${theme === 'custom' ? 'text-gray-400' : 'text-gray-500'
                        }`}>
    ...
    <div className="fixed bottom-6 right-6 z-40">
      <div className={`w-80 p-4 rounded-xl shadow-lg ${theme === 'custom' ? 'bg-gray-800 border-gray-700' : 'bg-white border'
        } border`}>
        <div className="flex items-center justify-between mb-3">
          <h4 className={`font-semibold flex items-center gap-2 ${theme === 'custom' ? 'text-white' : 'text-gray-900'
            }`}>
